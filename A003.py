...
fig_categoria = px.bar(vendas_categoria, x='Categoria', y='Valor Total', title='Vendas por Categoria', color='Categoria', color_discrete_sequence=px.colors.qualitative.Bold)
...
fig_regiao = px.bar(vendas_regiao, x='Região', y='Valor Total', title='Vendas por Região', color='Região', color_discrete_sequence=px.colors.qualitative.Pastel)
...
fig_faixa_etaria = px.bar(vendas_faixa_etaria, x='Faixa Etária', y='Valor Total', title='Vendas por Faixa Etária', color='Faixa Etária', color_discrete_sequence=px.colors.qualitative.Prism)
...
fig_meio_pagamento = px.pie(vendas_meio_pagamento, names='Meio de Pagamento', values='Valor Total', title='Vendas por Meio de Pagamento', color_discrete_sequence=px.colors.qualitative.Set3)
...
fig_sexo = px.bar(vendas_sexo, x='Sexo', y='Valor Total', title='Vendas por Sexo', color='Sexo', color_discrete_sequence=px.colors.qualitative.Safe)
...
fig_vip = px.bar(vendas_vip, x='Cliente VIP', y='Valor Total', title='Vendas por Cliente VIP', color='Cliente VIP', color_discrete_sequence=px.colors.qualitative.Dark2)
...
fig_tempo = px.line(vendas_tempo, x='Data', y='Valor Total', title='Tendências de Vendas ao Longo do Tempo', color_discrete_sequence=px.colors.qualitative.Vivid)
...
fig_mes = px.line(vendas_mes, x='Mes', y='Valor Total', title='Sazonalidade das Vendas', color_discrete_sequence=px.colors.qualitative.Pastel1)
...
fig_segmentacao = px.bar(segmentacao_clientes, x='Faixa Etária', y='Valor Total', color='Sexo', barmode='group', title='Segmentação de Clientes por Faixa Etária e Sexo', color_discrete_sequence=px.colors.qualitative.Set1)
...
fig_estado_civil = px.bar(vendas_estado_civil, x='Estado Civil', y='Valor Total', title='Vendas por Estado Civil', color='Estado Civil', color_discrete_sequence=px.colors.qualitative.Set3)
...
fig_box = px.box(df_filtered, x='Faixa Etária', y='Valor Total', color='Sexo', title='Distribuição de Vendas por Faixa Etária e Sexo', color_discrete_sequence=px.colors.qualitative.Pastel2)
